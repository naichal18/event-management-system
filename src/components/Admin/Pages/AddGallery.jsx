...
const AddGallery = () => {
  ...
  const getImageUrl = (imagePath) => {
    if (!imagePath) return 'https://via.placeholder.com/600x400?text=Gallery+Image';
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    if (!title || (!image && !preview) || !description) {
    ...
    try {
      ...
      formDataToSend.append('timeline', JSON.stringify(timelineSteps));
      if (image) {
    ...
    <div className="glass-panel" style={{ padding: '25px' }}>
      ...
      <form onSubmit={handleAdd} className="form-grid" style={{ marginBottom: '30px', maxWidth: '700px' }}>
        <div className="form-full">
          <label className="glass-label">Choose Gallery Image</label>
          <input
            type="file"
            accept="image/*"
